...
## Добавление в текст графических изображений

Для добавления графического изображения в репозиторий вводится команда

    ![Имя изображения в тексте](имя изображеня в папке)

Выглядит это так ![На уроке](prepod-i-studentyi.jpg) 

Поскольку графическое изображение относится к бинарным файлам, для Гита его отслеживание затруднительно, поэтому целесообразно вносить изображения в файл __*gitignore*__ - это служебный файл для игнорируемых файлов.
...
Для того, чтобы увидеть различия между последней сохраненной и текущей версиями, используется команда

    git diff

Для того, чтобы сравнить две конкретные версии, в указанной команде используются хеши этих версий

    git diff riuyt88 495kjsi

Сведения обо всех произведенных действиях в упрощенной форме в текущей версии вызываются командой

    git log --oneline

а по всему репозиторию

    git log --oneline --all

Отображение изменений в последовательности по разным веткам вызывается командой
...
Удаленый репозиторий это облачное хранилище ваших файлов. Позволяет работать в любой точке мира, где есть  Интернет
